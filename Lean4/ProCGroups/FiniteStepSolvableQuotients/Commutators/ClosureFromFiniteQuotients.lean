...
This module relates bounded commutator width to bounded word products and lifts uniform
finite-quotient bounds to closedness of the commutator subgroup.
-/
...
theorem mem_wordProducts_commutatorSet_of_isProductOfCommutatorsLE
  ...
  have hlenAux :
    ...
    induction l with
    | nil =>
        simp only [List.length_nil, Generation.wordProducts, List.map_nil, List.prod_nil,
...
/--
A bounded product of commutators in a quotient lifts to a bounded word product of commutators
upstairs.
-/
  ...
  have hmap :
    ...
    have hcommList :
        (L.map fun p : G × G => ⁅p.1, p.2⁆) =
          l.map (fun p => ⁅lift p.1, lift p.2⁆) := by
      dsimp [L]
      rw [List.map_map]
      rfl
    dsimp [y]
...
/--
A uniform finite-quotient commutator-width bound forces the commutator subgroup of a profinite
group to be closed.
-/
theorem isClosed_commutator_of_uniformFiniteQuotientCommutatorWidth
  ...
  have hclosure_subset :
    ...
    intro x hxcl
    refine (mem_closed_iff_forall_openNormal_quotient
      (G := G) (S := S) hSclosed).2 ?_
    intro U
