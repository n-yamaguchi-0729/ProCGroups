...
The word-product filtration is related to multiplication, inversion, and the abstract subgroup
generated by a set.  Under compactness hypotheses its finite levels are compact or closed, which
supports separation of a finite level by an open normal subgroup and later finite-quotient
...
omit [TopologicalSpace G] [IsTopologicalGroup G] in
/--
Multiplying elements from two word-product sets lands in the word-product set with summed
length.
-/
...
/-- Every bounded word-product set of a closed subset of a compact group is compact. -/
...
theorem exists_openNormalSubgroup_inf_eq_bot_of_finite
  ...
  have hsep :
    ...
    have hnotall : ¬ ∀ U : OpenNormalSubgroup G, (k : G) ∈ (U : Subgroup G) := by
      intro hkall
      have hkone : (k : G) = 1 :=
        ProfiniteGrp.eq_one_of_mem_all_openNormalSubgroups (G := G) hkall
      apply hk
      apply Subtype.ext
      simpa using hkone
  ...
  · refine ⟨topU, ?_⟩
    ...
    have hk_eq : k = 1 := by
      by_contra hk
      exact hs ⟨k, by simp only [ne_eq, Finset.mem_filter, Finset.mem_univ, hk,
          not_false_eq_true, and_self, s]⟩
