...
theorem ConvergesToOne.to_alongOpenSubgroups {X : Set G}
  ...
  have hbad : {x : X | (x : G) ∉ U}.Finite := by
    ...
    simpa only [Set.compl_setOf] using heventually
  have hsubset :
      X \ (U : Set G) ⊆
...
theorem ConvergesToOneAlongOpenSubgroups.to_convergesToOne
  ...
  have hbadU : {x : X | (x : G) ∉ U}.Finite := by
    letI : Finite ((fun x : X => (x : G)) '' {x : X | (x : G) ∉ U}) :=
      hbadImage.to_subtype
    exact Finite.Set.finite_of_finite_image _ (by
      intro a _ b _ hab
...
theorem topologicalRank_ne_zero [T1Space G] [Nontrivial G] :
  ...
  letI : IsEmpty X := Cardinal.mk_eq_zero_iff.mp (hcard.trans hzero)
  have hXempty : X = ∅ := by
    ext x
    constructor
    · intro hx
