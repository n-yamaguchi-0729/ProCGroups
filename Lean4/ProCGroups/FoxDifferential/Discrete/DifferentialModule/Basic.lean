...
/-- The group-ring scalar associated to `g` is the basis element supported at `ψ g`. -/
@[simp]
...
/--
The discrete Fox relation is the coefficient-ring-agnostic crossed-differential relation for the
group-ring coefficient homomorphism.
-/
@[simp]
...
/--
The generic universal crossed-differential module for the group-ring scalar homomorphism
associated to \(\psi\) is linearly equivalent to the discrete universal differential module
\(A_{\psi}\).
-/
