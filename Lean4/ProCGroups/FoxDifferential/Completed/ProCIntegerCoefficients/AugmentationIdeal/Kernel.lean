...
/--
The domain-target section sends a group element to the corresponding group-algebra basis
element.
-/
@[simp]
...
/--
A source basis element minus the chosen lift of its target image lies in the kernel augmentation
ideal.
-/
...
/--
Every group-algebra element differs from the chosen lift of its target image by an element of
the kernel augmentation ideal.
-/
...
/--
For a surjective group map, the kernel of R[A] \(\to\) R[B] is generated by the augmentation
generators attached to kernel f.
-/
...
/--
The algebraic ideal in \(\mathbb{Z}_C\llbracket G\rrbracket\) generated by completed
augmentation generators coming from the kernel of \(\psi\).
-/
...
kernel-augmentation generator in \(\mathbb{Z}_C\llbracket G\rrbracket\).
-/
...
/--
The open-image quotient map sends a group-like class to its corresponding finite-stage quotient
representative.
-/
@[simp 900]
...
/--
Kernel classes of the source-stage quotient map lift to actual elements in the kernel of
\(\psi\).
-/
...
/--
Any source-stage projection of a completed-kernel element is the projection of an element of the
algebraic completed kernel-augmentation ideal.
-/
...
kernel-augmentation ideal generated by \([n]-1\) for \(n \in \ker \psi\).
-/
